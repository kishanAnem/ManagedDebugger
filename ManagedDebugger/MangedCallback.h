...
public:	
	...
	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::FunctionRemapOpportunity(
	...
	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::CreateConnection(
		ICorDebugProcess *pProcess,
		CONNID dwConnectionId,
		WCHAR *pConnName);

	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::ChangeConnection(
		ICorDebugProcess *pProcess,
		CONNID dwConnectionId);

	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::DestroyConnection(
		ICorDebugProcess *pProcess,
		CONNID dwConnectionId);

	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::Exception(
		ICorDebugAppDomain *pAppDomain,
		ICorDebugThread *pThread,
	...
	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::ExceptionUnwind(
		ICorDebugAppDomain *pAppDomain,
		ICorDebugThread *pThread,
		CorDebugExceptionUnwindCallbackType dwEventType,
		DWORD dwFlags);

	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::FunctionRemapComplete(
		ICorDebugAppDomain *pAppDomain,
		ICorDebugThread *pThread,
		ICorDebugFunction *pFunction);

	HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback2::MDANotification(
		ICorDebugController *pController,
		ICorDebugThread *pThread,
		ICorDebugMDA *pMDA);

	//HRESULT STDMETHODCALLTYPE ICorDebugManagedCallback3::CustomNotification(ICorDebugThread *pThread, ICorDebugAppDomain *pAppDomain);
};
